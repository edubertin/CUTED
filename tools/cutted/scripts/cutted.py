        ...
        <div class="editor-preview">
          ...
          <div class="preview-bar">
            <div class="preview-controls">
              <button class="preview-play" data-preview-play type="button">Play</button>
              <button class="preview-volume" data-preview-volume type="button">Vol 20%</button>
            </div>
...
.editor-shell{display:grid;grid-template-columns:minmax(280px,520px) minmax(360px,1fr);gap:14px;padding:0 14px 14px}.editor-preview{display:grid;align-content:start;gap:10px}.media{position:relative;aspect-ratio:16/9;background:#000;max-height:72vh;overflow:hidden;border-radius:6px}.media video,.media img{width:100%;height:100%;object-fit:cover;display:block;background:#000;pointer-events:none}.placeholder{display:grid;place-items:center;height:100%;color:#777}.preview-bar{display:grid;gap:8px;padding:8px;border:1px solid #252525;border-radius:8px;background:#0a0a0a}.preview-controls{display:flex;gap:6px;align-items:center}.preview-play,.preview-volume{min-width:72px;background:#f4f4f4;color:#050505;border:1px solid #f4f4f4}.preview-volume{background:#191919;color:#ddd;border-color:#333}.card[data-preview-format=tiktok] .media,.card[data-preview-format=shorts] .media,.card[data-preview-format=instagram] .media{aspect-ratio:9/16}.card[data-preview-format=facebook] .media{aspect-ratio:4/5}.card[data-preview-format=youtube] .media{aspect-ratio:16/9}.preview-strip,.card-tabs{display:flex;gap:6px;flex-wrap:wrap}.preview-strip button,.card-tabs button{background:#191919;color:#ddd;border:1px solid #303030;padding:8px 10px}.preview-strip button.active,.card-tabs button.active{background:#f4f4f4;color:#050505;border-color:#f4f4f4}
...
function bindOverlayPlacement(card){
  ...
  surface.onclick = event => {
    if (event.target.closest("[data-overlay-drag]") || event.target.closest("[data-overlay-menu]") || event.target.closest("[data-preview-play]")) return;
...
function syncPreviewVolumeButton(card){
  ...
  button.hidden = false;
  button.textContent = video.muted ? "Mudo" : "Vol 20%";
}
